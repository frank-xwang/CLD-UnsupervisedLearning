...
class ContrastTrainer(BaseTrainer):
    ...
    def train(self, epoch, train_loader, model, model_ema, contrast,
        ...
        if args.mem == 'moco':
            if args.method == 'CLD':
                outs = self._train_moco_cld(epoch, train_loader, model, model_ema,
    ...
    def _train_moco_cld(self, epoch, train_loader, model, model_ema, contrast,
                    criterion, optimizer):
        """
        MoCo encoder style training. This needs two forward passes,
        one for normal encoder, and one for moco encoder
        ...
        for idx, data in enumerate(train_loader):
            ...
            # split into three crops
            x1, x2, x3 = torch.split(inputs, 3, dim=1)

            # shuffle BN for momentum encoder
            k, all_k = self._shuffle_bn(x3, model_ema)
            # k2, all_k2 = self._shuffle_bn(x4, model_ema)
            ...
            if args.jigsaw:
                ...
                bsz, m, c, h, w = inputs_jig.shape
                inputs_jig = inputs_jig.view(bsz * m, c, h, w)
                q, q1b2, q_jig, q2, q2b2 = model(x1, x2, inputs_jig)

                # Calculate loss for first sets of images
                output = contrast(q, k, q_jig, all_k) # memory/mem_moco.py
                losses, accuracies = self._compute_loss_accuracy(
                    logits=output[:-1], target=output[-1],
                    criterion=criterion)
                loss = (1 - args.beta) * losses[0] + \
                    args.beta * losses[1]
                # Calculate loss for second sets of images
                output2 = contrast(q2, k, q_jig, all_k) # memory/mem_moco.py
                losses2, accuracies2 = self._compute_loss_accuracy(
                    logits=output2[:-1], target=output2[-1],
                    criterion=criterion)
                loss2 = (1 - args.beta) * losses2[0] + \
                    args.beta * losses2[1]
                ...
                accuracies[0] = (accuracies[0] + accuracies2[0])/2
                accuracies[1] = (accuracies[1] + accuracies2[1])/2
                losses[0] = (losses[0] + losses2[0])/2
                losses[1] = (losses[1] + losses2[1])/2
            ...
            if args.local_rank == 0:
                if (idx + 1) % args.print_freq == 0:
                    ...
                    print('{3} Train: [{0}][{1}/{2}]\t'
                        ...
                        'l_J {loss_jig.val:.3f} ({loss_jig.avg:.3f})\t'
                        'a_J {acc_jig.val:.3f} ({acc_jig.avg:.3f})'.format(
                        epoch, idx + 1, len(train_loader), dt_string, batch_time=batch_time,
                        data_time=data_time, loss=loss_meter, acc=acc_meter,
                        loss_jig=loss_jig_meter, acc_jig=acc_jig_meter))
                    sys.stdout.flush()
    ...
    def grouping(self, eq1, eq2, T):
        criterion = nn.CrossEntropyLoss().cuda()

        logits1 = torch.mm(eq1, eq2.t())
        logits2 = torch.mm(eq2, eq1.t())
        
        # Create label for cross-entropy loss
        targets1 = torch.arange(logits1.size(0)).long().cuda()
        targets2 = torch.arange(logits2.size(0)).long().cuda()

        # Calculate loss
        loss1 = criterion(logits1.div_(T), targets1)
