...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.evaluate:
        validate(val_loader, model, criterion, args, train_loader)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch in [args.start_epoch, 0, 1, 10, 20, 30, 40, 50, 60, 70] or epoch>=80:
...
def validate(test_loader, model, criterion, args, train_loader):
    ...
    many_shot_tensor = label_counts >= 100
    medium_shot_tensor = (label_counts < 100) & (label_counts > 20)
    few_shot_tensor = label_counts <= 20
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(test_loader):
            ...
            corr = output.argmax(dim=1) == target

            many_mask_batch = many_shot_tensor[target]
            medium_mask_batch = medium_shot_tensor[target]
            few_mask_batch = few_shot_tensor[target]

            corr_many = corr[many_mask_batch]
            corr_med = corr[medium_mask_batch]
            corr_few = corr[few_mask_batch]
            many.update(corr_many.float().mean() * 100., corr_many.size(0))
            med.update(corr_med.float().mean() * 100., corr_med.size(0))
            few.update(corr_few.float().mean() * 100., corr_few.size(0))
